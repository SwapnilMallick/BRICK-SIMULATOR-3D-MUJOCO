...
def build_worldbody_xml(bounds: WorldBounds, layout: InventoryLayout) -> str:
    half_length = bounds.length * 0.5
    half_width = bounds.width * 0.5
    ...
    for brick_type in INVENTORY_ORDER:
        for brick_index, position in iter_inventory_positions(brick_type, bounds, layout):
            ...
            lines.append(build_brick_body_xml(brick_type, brick_id, position))

    lines.append("  </worldbody>")
    return "\n".join(lines) + "\n"


def build_scene_xml(bounds: WorldBounds, layout: InventoryLayout) -> str:
    asset_xml = build_asset_section()
    worldbody_xml = build_worldbody_xml(bounds, layout)
...
def main() -> None:
    ...
    xml = build_scene_xml(bounds, layout)
