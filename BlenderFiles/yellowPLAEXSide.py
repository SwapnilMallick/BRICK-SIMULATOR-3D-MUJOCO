...
# Procedural Yellow PLAEX Side brick rebuild.
# Feature set from the original script:
# - Base 2x2x2 body
# - Hollow top stud at center
# - One outward tab on +X
# - One outward tab on -X
# - One outward tab on +Y
# - One trapezoidal groove on -Y
# - One through-hole under the stud
# - One underside recess


OBJ_NAME = "PLAEXSide_2x2x2"
...
def build_outer_body():
    ...
    add_trapezoid_prism(bm, center_xy=(-3.4, 0.0), axis="x", outward_sign=-1)
    add_trapezoid_prism(bm, center_xy=(0.0, 3.4), axis="y", outward_sign=1)
...
def create_trapezoid_groove_cutter(name, center_xy, target_y, rotation_z_degrees):
    ...
    small_face_world_y = sum(
        (cutter.matrix_world @ Vector(cutter.data.vertices[i].co)).y
        for i in small_face_ids
    ) / 4.0
    cutter.location.y += (target_y - GROOVE_EPSILON_OPEN - small_face_world_y)
...
def subtract_side_groove(body_obj):
    cutter = create_trapezoid_groove_cutter(
        "TrapeziumCutter_NegY",
        center_xy=(0.0, -3.4),
        target_y=-(BODY_Y * 0.5),
        rotation_z_degrees=180,
    )
    apply_boolean(body_obj, cutter, "DIFFERENCE", "NegY_TrapeziumGroove")
...
def subtract_internal_cutters(body_obj, stud_inner_radius):
    ...
    through_cutter.name = "ThroughHoleCutter"
    apply_boolean(body_obj, through_cutter, "DIFFERENCE", "ThroughHole")
    ...
    recess_cutter.name = "BottomRecessCutter"
    apply_boolean(body_obj, recess_cutter, "DIFFERENCE", "BottomRecess")
...
def build_top_stud(material):
    stud_obj, stud_mesh = create_mesh_object("TopStud")
    bm_stud = bmesh.new()
    ...
    add_ring_section(
        ...
        z0=top_z0,
        z1=top_z1,
        center_xy=STUD_POSITION,
        segments=STUD_SEGMENTS,
        cap_top=True,
        cap_bottom=True,
    )

    bmesh.ops.remove_doubles(bm_stud, verts=bm_stud.verts, dist=0.0001)
    bmesh.ops.recalc_face_normals(bm_stud, faces=bm_stud.faces)
    bm_stud.to_mesh(stud_mesh)
    bm_stud.free()
    stud_mesh.update()
...
def main():
    ...
    subtract_side_groove(body)

    stud_inner_radius = STUD_RADIUS - STUD_WALL
    subtract_internal_cutters(body, stud_inner_radius)

    stud = build_top_stud(material)

    final_obj = join_objects([body, stud], OBJ_NAME, material)
