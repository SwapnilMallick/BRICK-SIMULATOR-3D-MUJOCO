...
# Procedural Orange PLAEX Long brick rebuild.
# Feature set:
# - Base 5x2x2 body
...
def ensure_material():
    ...
    if bsdf is not None:
        bsdf.inputs["Base Color"].default_value = (1.0, 0.1, 0.0, 1.0)
...
def subtract_side_grooves(body_obj):
    ...
    neg_cutter = create_trapezoid_groove_cutter(
        "TrapeziumCutter_NegX",
        center_xy=(-3.4, 0.0),
        target_x=-(BODY_X * 0.5),
        rotation_z_degrees=90,
    )
    apply_boolean(body_obj, neg_cutter, "DIFFERENCE", "NegX_TrapeziumGroove")
