...
MESH_PATH = BLENDER_DIR / "greenPLAEXLong.obj"

BRICK_POSITION = (0.0, 1.0, 5.0)
BRICK_RGBA = (0.20, 0.62, 0.33, 1.0)
...
def build_scene_xml() -> str:
    asset_xml = "  <asset/>\n"
    geom_xml = (
        f'      <geom name="green_plaex_long_geom" type="box" size="{format_vec(FALLBACK_HALF_EXTENTS)}" '
    ...
    if MESH_PATH.exists():
        asset_xml = (
            "  <asset>\n"
            f'    <mesh name="green_plaex_long_mesh" file="{MESH_PATH.relative_to(WORKSPACE_ROOT).as_posix()}"/>\n'
            "  </asset>\n"
        )
        geom_xml = (
            f'      <geom name="green_plaex_long_geom" type="mesh" mesh="green_plaex_long_mesh" '
            f'rgba="{format_vec(BRICK_RGBA)}"/>\n'
        )

    return f"""<mujoco model="green_plaex_long_preview">
...
{asset_xml}  <worldbody>
    ...
    <body name="PreviewGreenPlaexLongBrick" pos="{format_vec(BRICK_POSITION)}">
{geom_xml}    </body>
  </worldbody>
</mujoco>
"""
...
def write_scene(xml: str) -> Path:
    ...
    output_path = BUILD_DIR / "green_plaex_long_preview.xml"
    output_path.write_text(xml, encoding="utf-8")
    return output_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a standalone preview scene for the green PLAEX long brick.")
